...
function Notice() {
    ...
    const addNotice = async () => {
        if (!form.title || !form.description || !form.noticeDate) {
    ...
    const updateNotice = async () => {
        try {
            await API.put("/api/notice/" + editingId, form);
    ...
    // Helper filter function with optional chaining and null safety
    const filteredNotices = [...noticeList]
        .sort((a, b) => new Date(b.noticeDate || 0) - new Date(a.noticeDate || 0))
        .filter((item) => {
                ...
                <div className="notice-container">
                    ...
                    <div className="notice-grid">
                            ...
                            <div key={item.id} className="notice-card">
                                <div className="notice-header">
                                    ...
                                    {item.important ? (
                                        ...
                                        <span className="badge badge-normal">Notice</span>
                                    )}
                                </div>

                                <p className="notice-date">📅 {item.noticeDate || "N/A"}</p>
                                ...
                                {(item.description?.length || 0) > 120 && (
                                    <button
                                        ...
                                        onClick={() =>
                                            setExpandedNotice(
                                                expandedNotice === item.id ? null : item.id
                                            )
                                        }
                                    >
                                        {expandedNotice === item.id ? "Show Less" : "Read More"}
                                    </button>
                                )}

                                <div className="notice-actions">
                                    <button className="edit-btn" onClick={() => editNotice(item)}>
                                        Edit
                                    </button>
                                    <button
                        ...
                        {filteredNotices.length === 0 && (
                            <div className="no-data">📢 No Notices Available</div>
                        )}
                    </div>
                </div>
            </div>
